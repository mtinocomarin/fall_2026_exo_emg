...
int LDA_test(float *TestData, float *Wg, float *Cg, float *xmean, float *xstd)
{
	...
	int test_decision;
    static float addition_results[MAX_SAMPLES]; 
    static int index = 0; 
    ...
    // save addition results to array called addition_results
    if (index < MAX_SAMPLES) {
        addition_results[index] = tmp1[0];  
        index++; 
    }

    // Print the array when it reaches MAX_SAMPLES
    if (index == MAX_SAMPLES) {
        printf("Addition Results:\n");
        for (int i = 0; i < MAX_SAMPLES; i++) {
            printf("%f\n", addition_results[i]);
        }
        index = 0;  // Reset the index to start over if needed
    }
