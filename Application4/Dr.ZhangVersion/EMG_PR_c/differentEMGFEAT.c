...
void tdfeats(
    ...
    int   win_length,    // window length (samples)
    ...
    float deadzone_zc,   // now used as WAMP threshold (e.g. 0.01–0.03)
    float deadzone_turn, // UNUSED (kept for API compatibility)
    int   scale_mav,     // MAV scaling (e.g. 2)
    int   scale_zc,      // UNUSED (kept for API compatibility)
    int   tdfeatureN     // number of TD features per channel (here: 4 = MAV, WL, RMS, WAMP)
)
{
    int   Nsig = channel;
    ...
    /* --- mean-center the data: remove DC per channel --- */
    ...
    for (SigNum = 0; SigNum < Nsig; SigNum++) {
        ...
        float thr_wamp = deadzone_zc;   // amplitude threshold for WAMP

        /* first sample */
        float x_prev = DataSet[0 * channel + SigNum];
        float ax_prev = fabsf(x_prev);

        mav_sum += ax_prev;
        rms_sum += x_prev * x_prev;

        /* rest of samples */
        for (i = 1; i < win_length; i++) {
            float x = DataSet[i * channel + SigNum];
            float ax = fabsf(x);

            /* MAV and RMS accumulators */
            mav_sum += ax;
            rms_sum += x * x;

            /* Waveform length: sum |x[n] - x[n-1]| */
            wl_sum += fabsf(x - x_prev);

            /* Willison amplitude: count |x[n] - x[n-1]| > threshold */
            if (fabsf(x - x_prev) > thr_wamp) {
                wamp_cnt += 1.0f;
            }
        ...
        float mav  = mav_sum / (float)win_length;
        float rms  = sqrtf(rms_sum / (float)win_length);
        float wl   = wl_sum;                          // you can rescale if desired
        float wamp = wamp_cnt / (float)win_length;    // normalized count

        /* optional MAV scaling (keep old behavior) */
        if (scale_mav != 0) {
        ...
        Features[base + 0] = mav;   // Feature index: ch * 4 + 0
        Features[base + 1] = wl;    // Feature index: ch * 4 + 1
        Features[base + 2] = rms;   // Feature index: ch * 4 + 2
        Features[base + 3] = wamp;  // Feature index: ch * 4 + 3
    }
