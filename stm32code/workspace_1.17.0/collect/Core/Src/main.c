...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 3; i++){
            TestData[21 + i] = (float)ADC_VAL[i] + 0.1;
            if (indextemp + i < 30)  // Prevent array overflow
                Temp[indextemp + i] = TestData[21 + i];
        }
        indextemp += 3;
    }
}
...
{
    if (indextemp >= 30)  // Prevent exceeding limit
    {
        HAL_TIM_Base_Stop(&htim3);
        HAL_ADC_Stop_DMA(&hadc1);
        indextemp = 0;
    ...
    if (indext < 24)
    {
        for (int i = 0; i < 3; i++)
        {
            TestData[indext + i] = (float)ADC_VAL[i + 3] + 0.5;  // Fix: Correct indexing
            Temp[indext + i] = TestData[indext + i];
        }
        indext += 3;
        indextemp += 3;
    ...
    {
        // Shift left (remove oldest values)
        for (int i = 0; i < 21; i++)  // Fix: Stop at index 21 to prevent out-of-bounds
        {
            TestData[i] = TestData[i + 3];
        }
        ...
        // Add new values to the end
        for (int i = 0; i < 3; i++){
            TestData[21 + i] = (float)ADC_VAL[i + 3] + 0.5;
            if (indextemp + i < 30)  // Prevent array overflow
                Temp[indextemp + i] = TestData[21 + i];
        }
        indextemp += 3;
